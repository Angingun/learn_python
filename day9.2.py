...
class Card(object):
    ...
    def __str__(self):
        ...
        face_str_dict = {'1': 'A', '11': 'J', '12': 'Q', '13': 'K'}
        for key, value in face_str_dict.items():
            if str(self._face) == key:
                face_str = value
            else:
                face_str = '{:>2d}'.format(self._face)
...
def main():
    p = Poker()
    p.shuffle()
    players = [Player('abc'), Player('bcd'), Player('cde'), Player('def')]
    for _ in range(13):
